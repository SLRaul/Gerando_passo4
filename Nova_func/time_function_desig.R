...
periodo_trabalhado <- function(data_inicial, data_final, dias_mes, BD_desig ){
  ...
  BD_afastamentos <- periodo_trabalhado_afastamento(data_inicial, BD_afastamentos, BD_desig)
  ...
  BD_desig <- left_join(BD_desig, BD_serventias, "nome_serventia_desig") %>% select(nome_magis, inicio_desig, fim_desig, codigo_VT, nome_serventia_sicond,Tipo_magis, tempo_trabalhado, Dias_de_Afastamento)
  return(BD_desig)
}


#data inicial do mes de refer?ncia
data_inicial <- dmy("01/02/2020")
data_final <- dmy("29/02/2020")
dias_mes <- 29

#o retor ser? em rela??o as dias trabalhados
periodo_trabalhado(data_inicial, data_final, dias_mes, BD_desig )
